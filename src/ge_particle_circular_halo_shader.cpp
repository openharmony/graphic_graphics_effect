...
std::shared_ptr<Drawing::RuntimeShaderBuilder> GEParticleCircularHaloShader::GetParticleCircularHaloBuilder()
{
    ...
    if (ParticleCircularHaloShaderEffect_ == nullptr) {
        static constexpr char prog[] = R"(
            ...
            half4 fragColor; // output of the main function
            const float NUMBER_OF_SAMPLES = 20.;
            half colorScale = 1.2; // Change the light of the final color, see Line 209
            ...
            half noise(half2 st) {
                ...
                return mix(mix(dot(random2(i +2(0.0, 0.0)), f - half2(0.0, 0.0)),
                            dot(random2(i + half2(1.0, .0)), f - half2(1.0, 0.0)), u.x),
                        mix(dot(random2(i + half2(0.0, 1.0)), - half2(0.0, 1.0)),
            ...
            half2 haloRingShape(half2 uv, half2 position, half radius, half noiseVariation, half rotation) {
                uv = shapePerturbation(uv, noiseVariation, 30., 0.5);
                half2 polarCoords = half2((atan(uv.y, uv.x) + PI) / (2.0 * PI), length(uv-position));
                half radialBlurFactor = 0.1;//((polarCoords.y )*(5.0));
                half angularRandomVal = smoothRandomNoise11((polarCoords.x + radialBlurFactor)*13.0);
                ...
                half thickness = mix(0.001, 0.02, angularRandomVal) ;//+ (test*-abs(uv.x));
                half circleBorder = smoothstep(thickness, thickness*0.2, abs(polarCoords.y));
                // circleBorder = circleBorder * angularRandomVal;
                half halo = smoothstep(0.2, -0.2, abs(polarCoords.y))*2.0;
                // return clamp(half2(circleBorder, halo), 0.0, 1.0);
                return clamp(half2(circleBorder, halo), 0.0, 1.0);
            }
            ...
            half2 simpleHaloRingShape(half2 uv, half2 position, half radius, half noiseVariation, half haloThickness) {
                ...
                half thickness = mix(haloThickness, haloThickness*4.0, angularRandomVal) ; //+ (test*-abs(uv.x));
                half circleBorder = smoothstep(thickness, thickness*0.2, abs(polarCoords.y));
            ...
            half colorStopPos0 = 0.00;
            half colorStopPos1 = 0.22;
            half colorStopPos2 = 0.75;
            half colorStopPos3 = 0.90;
            half colorStopPos4 = 1.00;

            half3 colorStopColor0 = half3(255.0, 133.0, 127.0) / 255.0;   // FF717F
            half3 colorStopColor1 = half3(86.0,  146.0, 255.0) / 255.0;   // 5692FF
            half3 colorStopColor2 = half3(137.0, 240.0, 255.0) / 255.0;   // 89F0FF
            half3 colorStopColor3 = half3(255.0, 200.0, 161.0) / 255.0;   // FFC8A1
            half3 colorStopColor4 = half3(253.0, 216.0, 98.0) / 255.0;   // FDD862

            half3 getColorFromColorbar(half2 pt, half radius) {
                // Get colors for the particle-like halo, determined by UX
                half3 colorValue;
            ...
            // The belts movements were simulated using radial blur.

            half4 main(vec2 fragCoord) {
                // globalRadius = clamp(globalRadius, 0., 1.3);
                ...
                centeredUVs = centeredUVs - rotationCenter;
                ...
                half4 glow = clamp(pow(0.01 / abs(length(centeredUVs) - globalRadius *
