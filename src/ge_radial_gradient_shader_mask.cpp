    ...
    std::shared_ptr<Drawing::RuntimeShaderBuilder> builder) const
{
    ...
    size_t positionSize = param_.positions_.size(); // 0.01 minval
    if (colorSize <= 0 || colorSize > SIZE_ARRAY || colorSize != positionSize || width < 0.01 || height < 0.01) {
        return nullptr;
    }

    float color[SIZE_ARRAY] = {0.0}; // 0.0 default
    float position[SIZE_ARRAY] = {1.0}; // 1.0 default
    ...
    if (position[0] < 0.001) { // 0.001 represents the fraction bias
        position[0] -= 0.001;
    }
