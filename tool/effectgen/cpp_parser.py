...
class CppParser:
                ...
                if self._peek() and self._peek().type == 'attribute':
                    ...
                    if parsed.get('namespace') == 'ge' and parsed.get('function') == 'params':
                        ...
                        if parsed.get('namespace') and parsed.get('function'):
                            self._add_warning(
                                f"Skipping struct with attribute '{parsed.get('namespace')}::{parsed.get('function')}' "
                                f"(only 'ge::params' is parsed)",
                                attr_token.line, attr_token.column
                            )
                        # Restore position
                        self.pos = saved_pos
    ...
    def _parse_field(self, struct_name: str) -> Optional[FieldInfo]:
            ...
            if next_token.value == '}' and new_depth == 0:
                ...
                if not has_equals:
                    ...
                    for j in range(len(tokens_until_semicolon) - 1):
                        if (tokens_until_semicolon[j].type in ('keyword', 'identifier') and
                            j + 2 < len(tokens_until_semicolon) and
                            tokens_until_semicolon[j + 1].type in ('keyword', 'identifier') and
                            tokens_until_semicolon[j + 2].value == '('):
        ...
        for i in range(start_idx, len(tokens_until_semicolon)):
            ...
            if depth == 0 and token.type in ('keyword', 'identifier'):
                ...
                if (i + 2 < len(tokens_until_semicolon) and
                    tokens_until_semicolon[i + 1].type in ('keyword', 'identifier') and
                    tokens_until_semicolon[i + 2].value == '('):
            ...
            for i, token in enumerate(tokens_until_semicolon):
                if token.value in '<{([':
                    ...
                    if ident_count_at_depth_0 > 2:
                        self._add_error(
                            f"Missing semicolon in field declaration in struct '{struct_name}' "
                            f"(found '{token.value}' which may start a new field)",
        ...
        if first_token.type not in ('keyword', 'identifier'):
            self._add_error(
                ...
                f"expected type, got '{first_token.value}' ({first_token.type})",
        ...
        if not found_semicolon:
            self._add_error(
                ...
                tokens_until_semicolon[-1].line, tokens_until_semicolon[-1].column
            )
            return None
        ...
        for attr in attributes:
            ...
            if namespace == 'ge' and function == 'params':
                ...
                if current_prop_name:
                    prop_attributes.append(PropAttribute(
    ...
    def _process_range_attribute(self, params: Dict[str, Any], field_type: str,
                              line: int, column: int) -> Optional[RangeAttribute]:
        """
        ...
        if range_attr.min_value is None and range_attr.min_components is None and \
           range_attr.max_value is None and range_attr.max_components is None:
        ...
        if range_attr.min_components and range_attr.max_components:
            if len(range_attr.min_components) != len(range_attr.max_components):
                errors.append(
                    f"Min and max component count mismatch: "
                    f"{len(range_attr.min_components)} vs {len(range_attr.max_components)}"
                )

        if (range_attr.min_components and range_attr.max_value) or \
           (range_attr.min_value and range_attr.max_components):
            errors.append(
                "Inconsistent range specification: "
                "min and max must both be scalars or both be vectors"
            )
