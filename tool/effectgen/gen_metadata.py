...
def generate_transform_branch_static_asserts(structs: List[StructInfo], type_aliases: Dict[str, str]) -> str:
    """Generate static asserts to verify Transform branch selection at compile time."""
    output = []

    output.append("")
    output.append("// Compile-time branch verification for Transform and TransformCustomConvert")
    output.append("// These static asserts ensure the generated code follows expected transformation logic")
    output.append("")
    ...
    for struct in structs:
        for field in struct.fields:
            for tag_info in iterate_field_tags(struct, field):
                ...
                if field.prop_attributes and tag_info.prop_attr_index < len(field.prop_attributes):
                    prop_attr = field.prop_attributes[tag_info.prop_attr_index]
                    cast_from_type = prop_attr.cast_from
                    custom_transformer = prop_attr.custom
                ...
                if tag_info.is_array_element:
                    if field.prop_attributes and tag_info.prop_attr_index < len(field.prop_attributes):
                        prop_attr = field.prop_attributes[tag_info.prop_attr_index]
                        if prop_attr.array_accessor_type:
                            field_type = prop_attr.array_accessor_type
                        elif field_type.startswith("std::array<"):
                            match = re.match(r'std::array<([^,]+),\s*\d+>', field_type)
                            if match:
                                field_type = match.group(1).strip()

                from_type = cast_from_type if cast_from_type else field_type
                from_type = normalize_type(from_type, type_aliases)
                to_type = normalize_type(field_type, type_aliases)

                output.append(f"// {struct.name}::{field.name} -> {tag_info.tag_name}")
                output.append(f"// FromType: {from_type}, ToType: {to_type}")

                if cast_from_type:
                    if custom_transformer:
                        output.append(f"static_assert(")
                        output.append(f"    GetTransformConversionBranch<GEParamsMemberTag::{tag_info.tag_name}, {from_type}, {to_type}>() == GETransformBranch::CONVERT_HAS_CUSTOM,")
                        output.append(f"    \"Expected CONVERT_HAS_CUSTOM for {struct.name}::{field.name} (tag: {tag_info.tag_name})\");")
                    else:
                        if "shared_ptr" in to_type:
                            output.append(f"static_assert(")
                            output.append(f"    GetTransformConversionBranch<GEParamsMemberTag::{tag_info.tag_name}, {from_type}, {to_type}>() == GETransformBranch::CONVERT_CAST_FROM_SHARED_PTR,")
                            output.append(f"    \"Expected CONVERT_CAST_FROM_SHARED_PTR for {struct.name}::{field.name} (tag: {tag_info.tag_name})\");")
                        else:
                            output.append(f"static_assert(")
                            output.append(f"    GetTransformConversionBranch<GEParamsMemberTag::{tag_info.tag_name}, {from_type}, {to_type}>() == GETransformBranch::CONVERT_CAST_FROM_STATIC_CAST,")
                            output.append(f"    \"Expected CONVERT_CAST_FROM_STATIC_CAST for {struct.name}::{field.name} (tag: {tag_info.tag_name})\");")
                else:
                    output.append(f"static_assert(")
                    output.append(f"    GetTransformConversionBranch<GEParamsMemberTag::{tag_info.tag_name}, {from_type}, {to_type}>() == GETransformBranch::NO_CONVERT_SAME_TYPE,")
                    output.append(f"    \"Expected NO_CONVERT_SAME_TYPE for {struct.name}::{field.name} (tag: {tag_info.tag_name})\");")
