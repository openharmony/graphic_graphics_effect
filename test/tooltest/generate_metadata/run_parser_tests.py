...
class Validator:
    ...
    def __init__(self, test_dir: Path):
        ...
        self._prop_attribute_validators = self._get_dataclass_fields(
    ...
    def _validate_prop_attribute(
        self,
        validation: Dict[str, Any],
        context: ValidationContext,
        exp_prop: PropAttributeExpected,
        act_prop: Any,
    ):
        """Validate a single prop attribute."""
        self._validate_with_validators(
            validation, context, self._prop_attribute_validators, exp_prop, act_prop
        )
    ...
    def _validate_field(
        ...
        validation: Dict[str, Any],
        context: ValidationContext,
        exp_field: FieldExpectedResult,
        act_field: Any,
    ):
        """Validate a single field."""
        self._validate_with_validators(
            validation, context, self._field_validators, exp_field, act_field
        )
    ...
    def _validate_params(
        ...
        if exp_params != act_params:
            ...
            diff_keys = set(
                k
                for k in exp_params.keys() & act_params.keys()
                if exp_params[k] != act_params[k]
            )

            if missing_keys:
                self._add_difference(
                    validation, f"{context} params missing keys: {sorted(missing_keys)}"
                )
            if extra_keys:
                self._add_difference(
                    validation, f"{context} params extra keys: {sorted(extra_keys)}"
                )
    ...
    def _validate_struct(
        ...
        validation: Dict[str, Any],
        context: ValidationContext,
        exp_struct: StructExpectedResult,
        act_struct: Any,
    ):
        """Validate a single struct."""
        self._validate_with_validators(
            validation, context, self._struct_validators, exp_struct, act_struct
        )
    ...
    def validate_test_case(
        ...
        if sorted(actual_errors) != sorted(expected_errors):
            ...
            if missing_errors:
                self._add_difference(
                    validation, f"Missing expected errors: {sorted(missing_errors)}"
                )
            if extra_errors:
                self._add_difference(
                    validation, f"Unexpected errors: {sorted(extra_errors)}"
                )
        ...
        if test_case.should_parse and test_case.results:
            ...
            for i, (exp_struct, act_struct) in enumerate(
                zip(expected_structs, actual_structs)
            ):
    ...
    def validate_all(
        ...
        expected_results: Dict[str, TestCaseExpectedResult],
    ) -> List[Dict[str, Any]]:
...
class ResultFormatter:
    ...
    def _format_field_details(self, field: FieldInfo) -> List[str]:
        ...
        if not field.name or field.name == field.type:
            lines.append(
                f"      ⚠ WARNING: Field name appears to be missing or incorrect!"
            )
        if not field.type:
            lines.append(f"      ⚠ WARNING: Field type is empty!")
        return lines

    def _format_struct_details(
        self, test: TestCase, struct, struct_idx: int, total_structs: int
    ) -> List[str]:
    ...
    def print_test_details(self, test_cases: List[TestCase]):
        ...
        parsed_only_tests = [
            t for t in test_cases if t.status == TestStatus.PARSED_ONLY
        ]
        test_passed_tests = [
            t for t in test_cases if t.status == TestStatus.TEST_PASSED
        ]
        test_failed_tests = [
            t for t in test_cases if t.status == TestStatus.TEST_FAILED
        ]
        ...
        for test in test_cases:
            if test.should_parse and test.passed and test.results:
                for i, struct in enumerate(test.results):
                    lines = self._format_struct_details(
                        test, struct, i, len(test.results)
                    )
                    for line in lines:
                        print(line)
                ...
                if test.results:
                    print(
                        f"  Parsed {len(test.results)} struct(s) despite being invalid test:"
                    )
...
class TestRunner:
    ...
    def _parse_field_from_json(self, field_data: Dict[str, Any]) -> FieldExpectedResult:
        ...
        return FieldExpectedResult(
            ...
            prop_attributes=[
                self._parse_prop_attribute(pa)
                for pa in field_data.get("prop_attributes", [])
            ],
        )

    def _parse_struct_from_json(
        self, struct_data: Dict[str, Any]
    ) -> StructExpectedResult:
        ...
        return StructExpectedResult(
            ...
            fields=[
                self._parse_field_from_json(f) for f in struct_data.get("fields", [])
            ],
    ...
    def load_expected_results(self) -> bool:
        ...
        for test_case in self.test_cases:
            ...
            try:
                ...
                structs = [
                    self._parse_struct_from_json(s)
        ...
        try:
            ...
            if test_case.should_parse:
                if parsed_successfully:
                    ...
                    test_case.error = (
                        "Expected to parse successfully but got no structs"
                    )
            else:
                test_case.passed = True
                test_case.results = structs
    ...
    def validate_and_set_status(self):
        ...
        validation_results = self.validator.validate_all(
            self.test_cases, self.expected_results
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Parser robustness test runner - rewritten version"
    )
    parser.add_argument(
        "--save-expected",
        action="store_true",
    ...
    test_failed = sum(
        1 for t in runner.test_cases if t.status == TestStatus.TEST_FAILED
    )
